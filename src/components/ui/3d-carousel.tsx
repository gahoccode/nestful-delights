...
function ThreeDPhotoCarousel() {
  ...
  useEffect(() => {
    console.log("Bird images loaded:", cards)
  }, [cards])
      ...
      <AnimatePresence mode="sync">
        {activeImg && (
          <motion.div
            ...
            layout="position"
            onClick={handleClose}
            className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 m-5 md:m-36 lg:mx-[19rem] rounded-3xl"
            style={{ willChange: "opacity" }}
            transition={transitionOverlay}
          >
            <motion.img
              layoutId={`img-${activeImgIndex}`}
              src={activeImg}
              alt={`Enlarged swiftlet bird image ${activeImgIndex !== null ? activeImgIndex + 1 : ''}`}
